...
/**
 * unmarked server entry point. Wires the {@link ProgrammeLibrary}, the {@link JsonStore}
 ...
 */
public class UnmarkedServer
{
    private static final Logger LOG = LoggerFactory.getLogger(UnmarkedServer.class);
     ...
     */
    public static final String REQUEST_LOG_NAME = "org.mortbay.sailing.unmarked.requests";
    ...
    public static Server start(Path dataRoot, int port) throws Exception
    {
        ...
        UnmarkedConfig config = UnmarkedConfig.load(configDir.resolve("config.yaml"));
    ...
    /** Build version, from the Maven-filtered {@code unmarked.properties}. */
    static String version()
    {
        try (InputStream in = UnmarkedServer.class.getResourceAsStream("/unmarked.properties"))
        {
            if (in == null)
                return "unknown";
