package org.mortbay.sailing.unmarked.course;
...
import org.mortbay.sailing.unmarked.model.Position;
...
public class ProgrammeLibraryTest
{
    ...
    public void aRenameIsFollowedIntoTheLedgerOrEveryPublicationIsOrphaned(@TempDir Path root) throws Exception
    {
        ...
        org.mortbay.sailing.unmarked.store.CourseLedger ledger =
            new org.mortbay.sailing.unmarked.store.CourseLedger(root);
        ledger.start();
        ...
        ledger.publish("test.example", java.util.List.of(
            new org.mortbay.sailing.unmarked.store.CourseLedger.Publication(
    ...
    /**
     * A club's zone is found from ANY of its series, and that is what makes a record filed
     * by the right day.
     *
     * <p>{@code CourseRecord.series} is nullable — a boat may post a record naming only the club
     * and the course — and the zone is declared per programme file. Without the fallback such a
     * record is filed by UTC day, which is wrong for most of the world for part of every day,
     * and is reachable straight off the wire rather than only from a fixture.
     */
    @Test
    public void aClubsZoneIsFoundWithoutNamingItsSeries() throws Exception
    {
        ProgrammeLibrary library = new ProgrammeLibrary(
            java.nio.file.Paths.get("src/test/resources/testdata/config"));
        library.load();

        java.time.ZoneId exact = library.zone("test.example", "fixture");
        assertThat("the series it names", exact, notNullValue());
        assertThat("and with no series at all, the same club's zone",
            library.zone("test.example", null), is(exact));
        assertThat("...and for a series that has been retired since",
            library.zone("test.example", "gone-last-season"), is(exact));
        // Null only for a club this server has never heard of, which the store says out loud
        // rather than filing quietly on a day nobody can predict.
        assertThat("but nothing is invented for an unknown club",
            library.zone("somebody-else.example", null), nullValue());
    }
}
